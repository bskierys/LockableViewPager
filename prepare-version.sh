...
generate_changelog() {
	github_changelog_generator -u bskierys -p LockableViewPager --token ${auth_token} --include-labels bug,feature,ci --output ${changelog_file} --no-verbose
}

generate_release_notes() {
	github_changelog_generator -u bskierys -p LockableViewPager --token ${auth_token} --include-labels fixed --exclude-labels ci,task,invalid,testing --output ${temp_file} --simple-list --no-verbose
